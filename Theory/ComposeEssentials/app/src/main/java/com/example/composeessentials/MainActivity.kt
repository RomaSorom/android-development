...
//                }
                ...
                RecompositionTest()
                    ...
                    // перекомпозиція:
                    // 1. перевиклик функції з новим параметром
                    // 2. зміна внутрішнього стану функції (спричиняє 1 пункт у дочірніх елементах)

                var state: MutableState<String?> = remember { mutableStateOf(null) }
                // MutableState<T> - відстеження змін стану для перекомпозиції
                // .value - зберігає значення стану
                // remember - зберігає значення стану під час перекомпозиції
                // rememberSaveable - зберігає значенняс стану навіть при зміні конфігурації (поворот екрану)
                var state1: String by remember { mutableStateOf("String") }
                // by - оператор делегування, делегує доступ до .value
                // повертається не об'єкт MutableState, а значення, яке зберігається в .value


                // composable функції можуть виконуватися у будь-якому порядку
                    // пріоритетність обирає jetpack compose
                // composable функції можуть виконуватися паралельно
                // compose не перекомпозовує composable, які не взаємодіють зі станом
                // поточна перекомпозиція може прерватися, якщо виникає портреба в новій з новими параметрами
                // composable функція повинна виконуватися швидко, якщо вона буде виконуватися часто
                    // (приклад: використання анімацій у елементі, який буде швидко та часто викликатися)
            }
        }
    }
}
...
fun RecompositionTest() {
    ...
    var isBlack: Boolean by rememberSaveable { mutableStateOf(true) }
    Column(
